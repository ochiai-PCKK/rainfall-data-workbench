...
def test_load_style_profile_fills_x_axis_defaults(tmp_path) -> None:
    ...
    assert profile.day_boundary_offset_hours == 0.0
...
def test_load_style_profile_maps_legacy_zero_to_24(tmp_path) -> None:
    path = tmp_path / "style.json"
    path.write_text(json.dumps({"x_tick_hours_list": [0, 12]}), encoding="utf-8")
    profile = load_style_profile(path)
    assert profile.x_tick_hours_list == [12, 24]
