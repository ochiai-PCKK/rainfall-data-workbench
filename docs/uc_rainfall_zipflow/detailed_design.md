...
- Excelモードで候補一覧を表示
- 複数選択可能
- グラフ期間は `5日` / `3日前寄せ` / `3日中央` / `3日後寄せ` をユーザーが選択
- 未選択で実行した場合は入力エラー
...
- 時刻は `B列` を正として利用する
- 系列値は `Q列`（時間雨量）を利用する
- 5日選択時は120点全体を `plot_ref` へ渡す
- 3日前寄せ選択時は、`base_date-1日` を描画基準日として3日窓を切り出す
- 3日中央選択時は、`base_date` を描画基準日として3日窓を切り出す
- 3日後寄せ選択時は、`base_date+1日` を描画基準日として3日窓を切り出す
